...
public:
    ...
    void PrintList(); // Print to screen
...
// Insert item at the currPtr of list
template <class T>
void LinkedList<T>::InsertAt(const T& item)
{
    ...
    newNode = GetNode(item);	// create the new node
//    if (currPtr == rear)				// if list empty, insert at front
//       LinkedList::InsertRear();
//    if (currPtr == front)				// if list empty, insert at front
...
void LinkedList<T>::PrintList(){
	...
	while(currPtr != NULL){ // as long as it is not the last element in the list

        cout << currPtr->data <<  endl;
//		cout << currPtr->data << " , size is "<< size << " , position is "<< position << endl;
		currPtr = currPtr->NextNode();
